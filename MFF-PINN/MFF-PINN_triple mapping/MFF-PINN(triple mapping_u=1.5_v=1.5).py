...
def gen_traindata():
    data = loadmat("GFDM_u1.5_v1.5.mat")
    t = data["t"]
    x = data["x"]
    y = data["y"]
    q = data["q"]
...
net = dde.nn.STMsFFN(layer_size, activation, initializer, sigmas_x=[1], sigmas_t=[1,21,33])#To change
...
losshistory, train_state = model.train(epochs = 0, display_every = 1, callbacks = [pde_residual_resampler],model_restore_path = r"sigma21.33")#To change
...
np.savetxt("VIV_sigma21.33_ξ0.5.txt", R_nn)
...
np.savetxt("Yuntu_sigma21.33_u1.5_v1.5.txt", R)
np.savetxt("Yuntu_sigma21.33_u1.5_v1.5_true.txt", R1)
